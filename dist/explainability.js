...
exports.explainLex = explainLex;
exports.explainParse = explainParse;
exports.explainSemantics = explainSemantics;
...
function explainLex(result) {
    ...
    // Summary
    const tokenCounts = Object.values(token_1.TokenType)
        .filter((t) => t !== token_1.TokenType.EOF)
    ...
    if (result.recoveries.length > 0) {
        for (const r of result.recoveries) {
            if (r.strategy === 'Phrase-Level') {
                ...
                lines.push(`[PARSER] Recovery Strategy (Panic-Mode): ${r.message} Skipped ${r.skipped.type} ('${printableLexeme(r.skipped.lexeme)}').`);
            }
        }
    }
    if (result.ok) {
...
function formatSymbolTable(entries) {
    ...
    for (const e of entries) {
        lines.push(`  ${e.name} | type=${e.type} | value=${e.value} | width=${e.width} | level=${e.level} | offset=${e.offset}`);
    }
    return lines;
}
...
function explainSemantics(result) {
    ...
    for (const action of result.actions) {
        if (action.kind === 'typeCheck') {
            ...
            lines.push(`[SEMANTICS] ${verb} variable '${action.entry.name}' into Symbol Table (type=${action.entry.type}, offset=${action.entry.offset}).`);
        }
        else if (action.kind === 'bindClass') {
            lines.push(`[SEMANTICS] Declaring class '${action.entry.name}'.`);
        }
        else {
            lines.push(`[SEMANTICS] Declaring field '${action.entry.name}' in class '${action.className}' (type=${action.entry.type}, offset=${action.entry.offset}).`);
        }
    }
    if (result.ok) {
        ...
        const first = result.diagnostics[0];
        if (first)
            lines.push(`[SEMANTICS] FATAL ERROR: ${first.message}`);
