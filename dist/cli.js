...
function main() {
    ...
    const parseProgramResult = (0, parser_1.parseProgram)(result.tokens);
    const actualPattern = result.tokens
        .filter((t) => t.type !== token_1.TokenType.EOF)
        .map((t) => `[${t.type}]`)
        .join(' ');
    // eslint-disable-next-line no-console
    ...
    for (const r of parseProgramResult.recoveries) {
        if (r.strategy === 'Phrase-Level') {
            ...
            // eslint-disable-next-line no-console
            console.log(`[PARSER] Recovery Strategy (Panic-Mode): ${r.message} Skipped ${r.skipped.type} ('${r.skipped.lexeme}').`);
        }
    }
    ...
    if (parseProgramResult.ok) {
        ...
        const classTable = new classTable_1.ClassTable();
        let semOk = true;
        // eslint-disable-next-line no-console
        ...
        for (const statement of parseProgramResult.statements) {
            const semResult = (0, semantics_1.analyzeSemantics)(statement, symbolTable, classTable);
            for (const action of semResult.actions) {
                if (action.kind === 'typeCheck') {
                    ...
                    // eslint-disable-next-line no-console
                    console.log(`[SEMANTICS] ${verb} variable '${action.entry.name}' into Symbol Table (type=${action.entry.type}, offset=${action.entry.offset}).`);
                }
                    ...
                    // eslint-disable-next-line no-console
                    console.log(`[SEMANTICS] Declaring field '${action.entry.name}' in class '${action.className}' (type=${action.entry.type}, offset=${action.entry.offset}).`);
                }
            }
            if (!semResult.ok) {
                semOk = false;
